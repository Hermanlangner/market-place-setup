...
This document answers two automation questions from ecosystem research
(August 2026) rather than from scratch. Compose existing tools and keep only
thin glue.
...
| Client-side defense | [cc-plugin-audit](https://github.com/STRML/cc-plugin-audit) | Runs on engineers' machines as a plugin: hashes installed plugins, flags marketplace auto-updates at session start, surfaces security diffs. Not CI, but a complementary layer |
...
- The [community marketplace](https://github.com/anthropics/claude-plugins-community)
  ...
  review, nightly catalog sync, SHA-pinned entries). Nothing there is reusable,
  but it confirms the pattern: pin, scan, human gate.
...
- `claude plugin validate` checks structure and schema only, with no security
  scan.
...
- The PR body and diff are the audit trail. The `vetted` state is the merge
  itself.
...
- Our own AI second opinion (a Claude audit prompt) is optional on top. Treat
  any LLM verdict on untrusted content as advisory input to the human gate,
  never as the gate.
...
A bot is a Claude session without a human. In three of the four official
...
- **Bot behavior as a `bots/` category**: a PR-review bot, a triage bot, and
  the vendor-audit rubric, each a plugin entry. Updating a bot is a
  version-bump PR. Rolling back means pinning the old version. Vetting and
  telemetry are identical to human plugins.
...
- **Identity and telemetry**: run bots on service accounts with
  `user.slug=bot-<name>`. The `app.entrypoint` attribute separates cli, sdk,
  and action traffic, so bot cost and usage report through the same
...
- **[jamietanna/example-github-actions-sync-files](https://github.com/jamietanna/example-github-actions-sync-files)**.
  A minimal, complete Renovate and vendir setup syncing content from another
  repo (private source: […-private](https://github.com/jamietanna/example-github-actions-sync-files-private)).
...
Every claim below links to its source. The short version: Dependabot cannot see
our "dependency" (a subdirectory of an arbitrary git repo, pinned by tag) and
...
| Supply-chain delay before proposing updates | [minimumReleaseAge](https://docs.renovatebot.com/configuration-options/#minimumreleaseage): "suppress branch/PR creation for X days", which avoids yanked or briefly compromised releases | No equivalent |
...
Run it *alongside* Renovate for classic package ecosystems inside plugins. Bots
like [Mergify](https://mergify.com/) or [Kodiak](https://kodiakhq.com/) automate
*merging*, not update detection, so they are a different layer rather than an
alternative.

Two honesty notes. The submodules-break-the-marketplace point above is our own
analysis of Claude Code's clone behavior, not a documented statement. On the
hosted Renovate app, command execution is limited to what Mend's platform
allows, so unrestricted `postUpgradeTasks` requires self-hosting.
...
| Project | Mechanism | What to adopt |
| --- | --- | --- |
| **Firefox** | [`mach vendor`](https://firefox-source-docs.mozilla.org/mozbuild/vendor/index.html) + a [`moz.yaml`](https://wiki.mozilla.org/Moz_yaml) per vendored library + [updatebot](https://github.com/mozilla-services/updatebot) (cron every 6h) | Closest architectural match to our pipeline: per-library manifest with provenance, bot opens the patch and **assigns the named maintainer as reviewer**, plus an *alert-only* mode for upstreams too complex to auto-vendor |
...
| **conda-forge** | [autotick bot](https://conda-forge.org/blog/blog/tag/autotick-bot/) ([how it works](https://conda-forge.org/docs/maintainer/updating_pkgs.html)) | Watches PyPI/GitHub releases across thousands of feedstocks, computes the dependency graph, PRs each update, which proves the pattern scales |
...
These converge on five lessons:

1. A provenance manifest per vendored item.
2. A bot that detects and proposes, never merges.
3. A named human owner per item, auto-assigned.
4. Security notes recorded beside the code.
5. An alert-only mode for upstreams that cannot be automated.

### Claude-plugin-specific practice

- [Claude Code Plugins: From Personal Setup to Org Standard](https://claudefa.st/blog/tools/mcp-extensions/plugins-distribution). An org-level distribution write-up.
- [Chat2AnyLLM/awesome-claude-plugins](https://github.com/Chat2AnyLLM/awesome-claude-plugins). A curated list of marketplaces and plugins: curation by list, the low-tech ancestor of vendoring.
- [Publish a plugin to the marketplace](https://systemprompt.io/guides/publish-plugin-claude-marketplace) (systemprompt.io) and [Anthropic's plugins announcement](https://claude.com/blog/claude-code-plugins). The publisher-side view.
- Directories: [claudemarketplaces.com](https://claudemarketplaces.com/), [aitmpl.com/plugins](https://www.aitmpl.com/plugins/). Useful for discovering what exists, but not vetted.
...
| Renovate, hosted through the [Mend GitHub App](https://github.com/apps/renovate) | $0 (free tier covers private repos, and Mend sells enterprise tiers) | $0 (runs on Mend's infra) |
...
| Optional Claude second-opinion audit | — | measured in our lab at ~$0.06–0.27 per small run. A PR-sized run is roughly $0.5 to $2, so a few $/mo |
...
1. **Heeler does the scanning.** Its GitHub integration and Agent Skills
   inventory and scoring replace self-run skill-scanner *and* the client-side
   layer (cc-plugin-audit). It is already paid for.
2. **The hosted Renovate app does detection and PRs.** It is free and operated
   by Mend, so there is nothing to run or maintain. The only self-owned piece
   is the ~15-line vendir and renovate config from the reference repo above.
3. **GitHub does the rest.** Actions for `vendir sync` verification, and branch
   protection for the human gate.

Incremental spend is roughly Actions minutes plus optional Claude audit tokens.

Anti-recommendation: git **submodules plus Dependabot** looks even cheaper but
...
| skill-scanner (Cisco AI Defense) | SKILL.md security scan: YARA + patterns + adjudicating LLM + dataflow. SARIF, Action, and pre-commit | [GitHub](https://github.com/cisco-ai-defense/skill-scanner) · [docs](https://cisco-ai-defense.github.io/docs/skill-scanner) · [critique](https://repello.ai/blog/cisco-skill-scanner-alternatives) |
...
| claude-code-security-review (Anthropic) | AI PR review for trusted PRs only. Not injection-hardened, and not for vendored third-party code | [GitHub](https://github.com/anthropics/claude-code-security-review) |

### Commercial

| Vendor | Role | Lookup |
| --- | --- | --- |
| **Heeler** ("Agentic Development Security Platform") | The buy-side option closest to this whole doc. **Agent Skills Security** covers "instructions, skills, subagents, hooks and MCP configs inventoried across every assistant and scored 0–100", plus agent execution audit trails, MCP-embedded guardrails in Claude Code/Cursor/VS Code, and classic SCA/SAST/secrets with auto-remediation PRs. Continuous inventory and scoring across all machines, versus our pipeline's PR-time gate. Overlaps Renovate (SCA PRs) and skill-scanner (skill scanning) in one platform. SOC 2 Type II, pricing not public | [heeler.com](https://www.heeler.com/) · [funding/background](https://www.securityweek.com/application-security-startup-heeler-raises-8-5-million-in-seed-funding/) |
...
assistant. These are different control points. If the org adopts an ASPM
platform with agent-skills coverage, the PR-time gate stays as defense in
depth, and the client-side layer (cc-plugin-audit) becomes redundant.
