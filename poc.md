...
Same plumbing, different content. The POC changes only the top row of that
diff: it keeps the verified machinery untouched and gives it a small cast
worth watching. One catalog, a core everyone gets, two teams that own their
folders, shared scripts nobody copies, one vetted outsider, and a release
triggered mid-demo that the audience watches arrive in Claude Code.
...
GitHub form from the README's vetting section next to it. The audience sees
...
claude plugin update blue-set@acme     # + 1 dependency: party-parrot
...
Auto-update: yes, and the demo shows it once. The README already records
that bundle changes reach users through `plugin update` or auto-update, so
the runbook includes the hands-off variant: ship, open a fresh session,
the new skill is simply there. Every other update in the demo runs
explicitly, because on stage the update must land when the presenter says
so, not when a session happens to start. Pinning down the exact
auto-update trigger and any setting that controls it is a phase 2 verify
item.
...
```text
Act 1  add + install        marketplace add, install blue-set, watch deps resolve
Act 2  prove what loaded    standup, ping markers, plugin details team-blue
Act 3  browse the levers    /plugin by category, tags, handpick scout
Act 4  orchestration        relay race via team-green
Act 5  repo vs marketplace  tools/acme catalog, note it never installs
Act 6  the live update      ship.sh forward, timewarp back, celebrate
Act 7  guardrails           disable refused, uninstall --prune
```

Every block states the command and the line to look for, so a dry run is
just diffing reality against the file. Acts 1, 6, and 7 are lifted from the
README's already-verified test sequences; the runbook adds ordering and
expected output, it does not invent new mechanics.
...
## Phase 1: six lanes, fully parallel
...
| F | docs + runbook | `DEMO.md`, `README.md`, `docs/concepts-and-layout.md`, `mise.toml` | contract |

No two lanes touch the same file. Lane A writes catalog entries from the
contract without waiting to see the plugins land. Lane F rewrites the
`team-a` references (README verify steps, `lab:ping`, concepts doc tree)
against the contract names. Old placeholder folders are deleted in lane A's
commit since the catalog is what makes them live.
...
merge lanes
  ...
  claude plugin install blue-set@acme         expect + 2 deps (core-basics, shared-kit)
  walk DEMO.md acts 1-5 and 7 locally         every 🏓 marker checked
  ship + fresh session                        pin down the auto-update trigger
...
## Phase 3: publish and rehearse (sequential, needs the open question answered)
